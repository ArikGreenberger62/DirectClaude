/* main.c — 16-ignition-led
 *
 * Reads ADC1 IgnitionPCO (PC0 / ADC1_INP10 / CH10 / Rank 3 / DMA[2])
 * via continuous DMA and controls LED_R (PC8) and LED_G (PC9):
 *   ADC raw > 300  →  LED_R ON,  LED_G OFF   (ignition present)
 *   ADC raw <= 300 →  LED_G ON,  LED_R OFF   (no ignition)
 *
 ...
 *   [IGN] raw=XXXX  LED_R=ON  LED_G=OFF
 *   [IGN] raw=XXXX  LED_R=OFF LED_G=ON
 *
...
int main(void)
{
    ...
    const char banner[] =
        ...
        " | LED_R=PC8 | LED_G=PC9 | P3V3_SW_EN=PC11 HIGH\r\n";
    ...
    while (1)
    {
        ...
        {
            /* Ignition present: LED_R ON, LED_G OFF */
            HAL_GPIO_WritePin(LED_R_GPIO_Port, LED_R_Pin, GPIO_PIN_RESET);
            HAL_GPIO_WritePin(LED_G_GPIO_Port, LED_G_Pin, GPIO_PIN_SET);
        }
        else
        {
            /* No ignition: LED_G ON, LED_R OFF */
