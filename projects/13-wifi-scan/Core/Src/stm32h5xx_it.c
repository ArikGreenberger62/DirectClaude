/* stm32h5xx_it.c — Interrupt Service Routines for 13-wifi-scan.
 *
 * Copied from 12-ble-detect pattern; all peripheral ISRs retained so that
 * enabled NVIC vectors have valid handlers (no hard fault on spurious IRQs).
 * UART9_IRQHandler is required for FC41D interrupt-driven RX.
 */

#include "main.h"
#include "stm32h5xx_it.h"

/* External peripheral handles used by ISR dispatch ─────────────────────── */
...
/* SysTick is unused — HAL tick comes from TIM4. */
