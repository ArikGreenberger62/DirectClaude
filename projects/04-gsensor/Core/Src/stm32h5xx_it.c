...
/* SysTick_Handler is empty: HAL tick comes from TIM4 (stm32h5xx_hal_timebase_tim.c) */
...
{
  HAL_GPIO_EXTI_IRQHandler(MC60_RI_Pin);
}

void EXTI5_IRQHandler(void)
{
  HAL_GPIO_EXTI_IRQHandler(CAN_FD_INTn_Pin);
}

void EXTI8_IRQHandler(void)
{
  HAL_GPIO_EXTI_IRQHandler(IGNITION_DIG_Pin);
}
 ...
 * calibration.c uses __builtin_sqrtf / __builtin_fabsf which avoids libm,
 * but the stub is kept here as a safety net (identical pattern to proj03). */
int *__errno(void)
{
