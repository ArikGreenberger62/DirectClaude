...
## ST_IOT reuse pattern (template for new projects)
Copied + edited locally:
- `Core/Src/spi.c` — SPI1 Mode 3, prescaler 16, **AFCNTR=1**.
- `Core/Src/stm32h5xx_it.c` — adds `__errno` stub.
- `Core/Src/main.c` — uses ST_IOT `SystemClock_Config`; adds LED + app loop.

All other ST_IOT sources referenced via `${LL}/...` in CMakeLists.
